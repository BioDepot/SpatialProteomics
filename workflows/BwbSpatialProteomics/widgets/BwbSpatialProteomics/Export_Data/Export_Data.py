...
class OWExport_Data(OWBwBWidget):
    name = "Export_Data"
    description = "None"
    priority = 10
    icon = getIconName(__file__,"qupath_icon.jpeg")
    want_main_area = False
    docker_image_name = "qupath"
    docker_image_tag = "latest"
    inputs = [("qupathdir",str,"handleInputsqupathdir")]
    ...
    optionsChecked=pset({})
    qupathdir=pset(None)
    Image Name=pset(None)
    def __init__(self):
        super().__init__(self.docker_image_name, self.docker_image_tag)
        with open(getJsonName(__file__,"Export_Data")) as f:
