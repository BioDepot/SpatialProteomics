...
class OWLaunch_QuPath(OWBwBWidget):
    name = "Launch_QuPath"
    description = "View QuPath GUI"
    priority = 1
    icon = getIconName(__file__,"qupath_icon.jpeg")
    want_main_area = False
    docker_image_name = "brycenofu/qupath"
    docker_image_tag = "0.5.1"
    inputs = [("qupathdir",str,"handleInputsqupathdir"),("qpProjFile",str,"handleInputsqpProjFile"),("imageFileName",str,"handleInputsimageFileName")]
    ...
    imageFileName=pset(None)
    def __init__(self):
        super().__init__(self.docker_image_name, self.docker_image_tag)
        with open(getJsonName(__file__,"Launch_QuPath")) as f:
    ...
    def handleInputsqpProjFile(self, value, *args):
        if args and len(args) > 0: 
            ...
            self.handleInputs("inputFile", value, None, False)
    def handleInputsimageFileName(self, value, *args):
        if args and len(args) > 0: 
            self.handleInputs("imageFileName", value, args[0][0], test=args[0][3])
        else:
            self.handleInputs("inputFile", value, None, False)
